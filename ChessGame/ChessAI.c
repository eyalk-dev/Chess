...
void doCompTurn(ChessGame* game, bool print) {
    ...
    Piece* piece = cellFromCoord(&choice.nextPos, &game->gameBoard)->piece;
    ...
    if (print) {
        ...
        switch (game->gameState) {
            ...
            case GAME_STATE_ON:
                if (isKingThreatened(game, game->settings.user1Color)) {
        ...
        for (int j = 0; j < game->gameBoard.dims[1]; j++) {
            ...
            if (piece != NULL && piece->color == game->currPlayer) {
                ...
                for (int k = 0; k < ArrayListSize(optionalsMoveCoords); k++) {
                    ...
                    piece = cellFromCoord(nextPos, &game->gameBoard)->piece; // update because piece can be destroyed
