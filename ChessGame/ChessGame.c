...
ChessGame* createGame(void) {
	...
	if (board == NULL) { // allocation of elements on board failed
		destroyAllElementsInBoard(&game->gameBoard); // TODO check function meaning if board == NULL
		free(game);
		return NULL;
	}
...
void initializeGame(ChessGame* game) {
	...
	game->settings.difLevel = DEFAULT_DIFFICULTY;
...
COMMAND doPlayerTurn(ChessGame* game) {
	...
	while (true) {
		...
		case COMMAND_MOVE:
            successfulMove = move(game, cmdPack);
			if (successfulMove) return cmd;
			else continue;
...
bool move(ChessGame* game, Command cmd) {
	...
	if (!cmd.isValidArg) {
		Color color = game->currPlayer;
		printMsgForPlayingStateAndUserTurn(MSG_GAME_USER_CMD_MOVE_XY_TO_IJ_1_INVALID_POS,
                                           __func__,color, &currPos, &nextPos);
		return false;
	} else {
        /// doing move
...
bool moveAdvanced(ChessGame* game, const Vector* prevPos, const Vector* nextPos, bool print) {
	...
	if (!isExistsPieceColor(prevPos, board, game->currPlayer)) {
		if (print)
			printMsgForPlayingStateAndUserTurn(MSG_GAME_USER_CMD_MOVE_XY_TO_IJ_2_XY_NOT_PLAYER_PIECE,
                                               __func__,playerColor, prevPos, nextPos);
		return false;
	}
	...
	// Doing Move (it's probably legal unless its expose the player's king)

	// remove piece from previous pos
	Piece* movingPiece = cellFromCoord(prevPos, board)->piece;
	removePiece(movingPiece, board);

	// destroy piece in dest pos, if exists
	PieceType* killedPieceType = NULL;
    Piece* destPiece = cellFromCoord(nextPos, board)->piece;
	if (destPiece != NULL) {
	...
	// make sure move didn't make its own king vulnerable
	if (isKingThreatened(game, playerColor)) {
		if (print)
			printMsgForPlayingStateAndUserTurn(MSG_GAME_USER_CMD_MOVE_XY_TO_IJ_3_MOVE_ILLEGAL_FOR_PLAYER_PIECE,
                                               __func__, playerColor, prevPos, nextPos);
    ...
    // move don't make its own king vulnerable
    // move is legal
    if (destPiece != NULL) {
        destroyPiece(destPiece, &game->gameBoard); // prevent memory leaks
    }
    // update history
	...
	if (print) {
		...
		case GAME_STATE_ON:
			if (isKingThreatened(game, game->currPlayer)) {
...
bool isMoveValid(ChessGame* game, Vector* currPos, Vector* nextPos) {
    ...
    Piece* movingPiece = cellFromCoord(currPos, board)->piece;
...
void doingGetMoves(ChessGame* game, Command cmd) {
    assertAndPrintFuncErr(game != NULL, __func__);

    Vector pos = {{cmd.currPos[0], cmd.currPos[1]}};
    GameBoard* board = &game->gameBoard;
    Color color = game->currPlayer;

    if (!cmd.isValidArg || !isCoordInBoard(&pos, board)) {
        printMsgForPlayingStateAndUserTurn(MSG_GAME_USER_CMD_GET_MOVES_1_INVALID_POS,
                                           __func__, color, NULL, NULL);
        return;
    }

    if (!isExistsPieceColor(&pos, board, color)) {
        printMsgForPlayingStateAndUserTurn(MSG_GAME_USER_CMD_GET_MOVES_2_INVALID_POS,
                                           __func__, color, NULL, NULL);

        return;
    }

    // function is valid, we can get moves now
    ArrayList* allValidMoves = getValidMoves(game, &pos);

    for (unsigned int i=0; i < allValidMoves->actualNumElems; i++) {
        Vector* move = (Vector*) ArrayListGetAt(allValidMoves, (int) i);
        if (move == NULL) return;
    }

    // TODO sort valid moves ascending

    // TODO check capture opponent pieces and being threatened (in 1-2 difficulty)
    if (game->settings.difLevel <= 2) {

    }

    // TODO add castling

    // TODO print optional moves

    ArrayListDestroy(allValidMoves);
}


ArrayList* getValidMoves(ChessGame* game, Vector* coord) {

    Piece* piece = cellFromCoord(coord, &game->gameBoard)->piece;

    assertAndPrintFuncErr(piece != NULL, __func__);

	if (piece == NULL) return NULL;

	ArrayList* optionalMovesCoords = getAllPathReport(coord, &game->gameBoard,
...
void undoingMove(ChessGame* game, bool print) {
	...
	Piece* piece = cellFromCoord(&choice->nextPos, &game->gameBoard)->piece; // get piece to undo its move
	assertAndPrintFuncErr(piece != NULL, __func__);
	...
	assertAndPrintFuncErr(cellFromCoord(&choice->prevPos, &game->gameBoard)->piece == NULL, __func__); // assert the previous position is empty
	putPiece(piece, &game->gameBoard, &choice->prevPos);
	if (history->type != NULL) {
		Piece* revivedPiece = initializePiece(history->type, &game->gameBoard, &choice->nextPos,
				getOppositeColor(piece->color)); // bring back killed piece, if existed
		if (revivedPiece == NULL) { // allocation error
	...
	game->gameState = GAME_STATE_ON; // can undo after game finished TODO verify
}
...
bool isKingThreatened(ChessGame* game, Color kingColor) {
...
bool isPlayerCanMove(ChessGame* game, Color color) {
	...
	for (int col = 0; col < game->gameBoard.dims[0]; col++) {
		for (int row = 0; row < game->gameBoard.dims[1]; row++) {
			...
			if (!isCellEmpty(cell)) {
				// check if it's relevant Player's piece
