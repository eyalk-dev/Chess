...
ArrayList* initializePieceTypes(void) {
...
void loadTable(ChessGame* game, char table[CHESS_N_ROWS][CHESS_N_COLUMNS]) {

	// clean board from pieces
    destroyBoardPieces(&game->gameBoard);
    for (int y = 0; y < CHESS_N_ROWS; y++) {
        for (int x = 0; x < CHESS_N_COLUMNS; x++) {
			PieceType* type = getPieceTypeFromSymbol(game->gameBoard.pieceTypes, table[y][x]);
			if (type != NULL) {
                Color color = getColorFromSymbol(table[y][x]);
				Vector vec = {{x, y}};
...
PieceType* getPieceTypeFromSymbol(ArrayList* pieceTypes ,char symbol) {
    ...
    for (int i = 0; i < ArrayListSize(pieceTypes); i++) {
        ...
        if (symbol == whiteSymbol || symbol == toupper(whiteSymbol)) {
            return type;
        }
    }
    return NULL;
...
void setKings(ChessGame* game) {
	for (int row = 0; row < CHESS_N_ROWS; row++) {
		for (int col = 0; col < CHESS_N_COLUMNS; col++) {
			Vector vec = {{row, col}};
			Cell* cell = cellFromCoord(&vec, &game->gameBoard);
			if (!isCellEmpty(cell)) {
...
PieceType createPawnType(void) {
    ...
    Stride stride3 = {DIR_XY_UP, 2, true, stepHandlerPawn2UpOpening};
...
PieceType createBishopType(void) {
    int movesNum = 1;
    ArrayList* moveSet = ArrayListCreate(movesNum, sizeof(Move));
    if (moveSet == NULL)
        return (PieceType) NULL_PIECE_TYPE;
...
PieceType createKnightType(void) {
    int movesNum = 2;
    ArrayList* moveSet = ArrayListCreate(movesNum, sizeof(Move));
    if (moveSet == NULL)
        return (PieceType) NULL_PIECE_TYPE;
    ...
    Stride stride1_1 = {DIR_XY_UP, 2, true, stepHandlerNoThreatCanSkip};
...
PieceType createQueenType(void) {
    int movesNum = 1;
    ArrayList *moveSet = ArrayListCreate(movesNum, sizeof(Move));
    if (moveSet == NULL)
        return (PieceType) NULL_PIECE_TYPE;
...
void stepHandlerOnlyEmptyCellsNonSkip(StepDialog *stepDialog, const GameBoard *board){
    ...
    if (!isCellEmpty(cellFromCoord(&stepDialog->nextCoord, board)))
...
void stepHandlerPawn2UpOpening(StepDialog* stepDialog, const GameBoard* board){
    assertAndPrintFuncErr(stepDialog != NULL && board != NULL, __func__);
    Piece* piece = cellFromCoord(&stepDialog->origin, board)->piece;
